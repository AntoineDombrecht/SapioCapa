...
Une suite d'actions est possible si l'on sépare chacune d'entre elle par une virgule sans laisser d'espace, exemple : ```Marquer comme lu,déplacer vers,supprimer```. <br />
...
Veillez à toujours étendre les tableaux plutôt que de les diminuer. Si vous le diminuez verifiez que les cellules du tableau ne soient pas verrouillées.<br />
```diff 
- Attention
```
